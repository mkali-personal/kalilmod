...
Usage:  python serve.py [--port 8000] [--no-browser] [--static] [--ephemeral]
...
Modes:
  ...
  --ephemeral (orthogonal to the above) - never persist per-user progress: GET
      /api/progress always returns {} and POST /api/progress is a no-op. Used for
      share bundles that live in a *shared* folder, so one student's position
      never leaks to the next (F5 resets, which is fine for short lessons) and
      nothing is written to the shared/read-only folder. Static share bundles
      default to this (see tools/build_bundle.py).
...
EPHEMERAL = False    # set from --ephemeral: don't persist progress (shared bundles)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if url.path == "/api/mode":
            ...
            if EPHEMERAL:
                self._send_json(200, {})  # never persisted; every load starts fresh
            else:
    ...
    def do_POST(self):
        ...
        try:
            ...
            return
        if EPHEMERAL:
            # Accept and validate the body, but persist nothing: shared bundles keep
            # each student's progress only in their own live tab (see --ephemeral).
...
def main():
    ...
    parser.add_argument("--ephemeral", action="store_true",
                        help="don't persist progress (for shared-folder bundles)")
    args = parser.parse_args()

    global MODE, EPHEMERAL
    MODE = "static" if args.static else "dynamic"
    EPHEMERAL = args.ephemeral

    server = ThreadingHTTPServer(("127.0.0.1", args.port), Handler)
    url = f"http://127.0.0.1:{args.port}/"
    tag = MODE + (", ephemeral (progress not saved)" if EPHEMERAL else "")
    print(f"Kalilmod running in {tag} mode at {url}  (Ctrl+C to stop)")
